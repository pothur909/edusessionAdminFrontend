...
export default function LeadsList() {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'new' | 'contacted' | 'converted' | 'not_interested' | 'demo_scheduled' | 'demo_completed' | 'demo_cancelled' | 'demo_no_show' | 'demo_rescheduled' | 'demo_rescheduled_cancelled' | 'demo_rescheduled_completed' | 'demo_rescheduled_no_show' | 'no_response_from_Lead'>('all');
  ...
  const fetchLeads = async () => {
    try {
      const response = await fetch(`http://localhost:6969/api/leads/viewleads${activeTab !== 'all' ? `?status=${activeTab}` : ''}`);
  ...
  const handleDeleteLead = async (leadId: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:6969/api/leads/deleteled/${leadId}`, {
        method: 'DELETE',
      });
                      ...
                      return (
                        <tr key={lead._id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <div className="text-sm text-gray-500">Source: {lead.leadSource}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
