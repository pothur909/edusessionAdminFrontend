...
//       try {
//         const response = await fetch(`http://localhost:6969/api/subject/${subjectToRemove._id}`, {
//           method: 'DELETE',
//         });
...
//         const studentResponse = await fetch(`http://localhost:6969/api/students/${studentId}`);
...
//           try {
//             const subjectsResponse = await fetch(`http://localhost:6969/api/subject/${studentId}`);
...
//             subjectResponse = await fetch(`http://localhost:6969/api/subject/${subject._id}`, {
//               method: 'PUT',
...
//           } else {
//             // Create new subject
//             subjectResponse = await fetch('http://localhost:6969/api/subject/add', {
//               method: 'POST',
//               headers: { 'Content-Type': 'application/json' },
//               body: JSON.stringify(subjectData),
//             });
...
//         // Fetch student enrollment data
//         console.log('Fetching student with ID:', studentId);
//         const studentUrl = `http://localhost:6969/api/students/${encodeURIComponent(studentId)}`;
...
//           // Fetch subjects for this student
//           try {
//             const subjectsUrl = `http://localhost:6969/api/subject/${encodeURIComponent(studentId)}`;
...
//               }
//             } else {
//               // If subjects fetch fails, just log it but don't throw error
...
//           } catch (error) {
//             console.error('Error fetching subjects:', error);
//             // Don't throw error, just set empty subjects
//             setSubjects([]);
//           }
//         } else {
//           throw new Error('Student not found');
...
//   }, [studentId]);
    ...
    // Fixed useEffect for fetching subjects
// useEffect(() => {
//   const fetchData = async () => {
//     // Skip if studentId is invalid
//     if (!studentId || typeof studentId !== 'string' || !/^[0-9a-fA-F]{24}$/.test(studentId)) {
...
//         // Fetch subjects for this student - FIXED URL STRUCTURE
//         try {
//           // Try multiple possible endpoint structures
//           const possibleUrls = [
//             `http://localhost:6969/api/subject/student/${encodeURIComponent(studentId)}`,
//             `http://localhost:6969/api/subjects/student/${encodeURIComponent(studentId)}`,
//             `http://localhost:6969/api/subject?studentId=${encodeURIComponent(studentId)}`,
//             `http://localhost:6969/api/subjects?student=${encodeURIComponent(studentId)}`,
//             `http://localhost:6969/api/subject/${encodeURIComponent(studentId)}`
//           ];
...
//             try {
//               console.log('Trying subjects URL:', url);
//               const subjectsResponse = await fetch(url);
              
//               if (subjectsResponse.ok) {
//                 const responseData = await subjectsResponse.json();
...
//                 } else if (responseData.subjects) {
//                   subjectsData = responseData.subjects;
//                 } else {
//                   subjectsData = responseData;
//                 }
...
//               }
//             } catch (urlError) {
//               console.log('Failed with URL:', url, urlError);
//               continue;
//             }
//           }

//           if (subjectsData && Array.isArray(subjectsData)) {
//             console.log('Successfully fetched subjects from:', successUrl);
...
//             setSubjects(transformedSubjects);
//           } else {
//             console.warn('No subjects found or invalid response structure');
//             setSubjects([]);
//           }
//         } catch (error) {
//           console.error('Error fetching subjects:', error);
...
// // Alternative: Add a separate function to fetch subjects that you can call manually
// const fetchSubjectsManually = async () => {
//   if (!studentId) return;
  
//   try {
//     console.log('Manually fetching subjects for student:', studentId);
    
//     // Add debug information
//     const debugUrls = [
//     //   `http://localhost:6969/api/subject/student/${studentId}`,
//     //   `http://localhost:6969/api/subjects/student/${studentId}`,
//     //   `http://localhost:6969/api/subject?studentId=${studentId}`,
//       `http://localhost:6969/api/subjects?student=${studentId}`
//     ];
...
//             const transformedSubjects = subjectsArray.map((subject: any) => ({
//               _id: subject._id,
//               student: subject.student,
//               board: subject.board || '',
//               class: subject.class || '',
...
//             setSubjects(transformedSubjects);
//             console.log('Successfully set subjects:', transformedSubjects);
//             return;
//           }
//         } else {
...
//     console.log('No working URL found for fetching subjects');
//   } catch (error) {
//     console.error('Error in fetchSubjectsManually:', error);
//   }
// };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (studentData) {
        ...
        try {
          const subjectsUrl = `http://localhost:6969/api/subject/student/${encodeURIComponent(studentId)}`;
    ...
    try {
      ...
      try {
        ...
        for (const subject of subjects) {
          ...
          if (subject._id) {
            // Update existing subject
            const updateUrl = `http://localhost:6969/api/subject/${encodeURIComponent(subject._id)}`;
            ...
            // Create new subject
            const createUrl = 'http://localhost:6969/api/subject/add';
