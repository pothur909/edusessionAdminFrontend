...
interface TeacherData {
  ...
  note: string;
  resumeLink: string;
  status: 'new' | 'interview' | 'interviewdone' | 'contacted' | 'rejected' | 'rejectafterinterview' | 'appointed';
}
...
export default function TeacherForm({ initialData, onSuccess, onError, isEdit = false }: TeacherFormProps) {
  ...
  useEffect(() => {
    if (formData.board.length > 0) {
      ...
      const selectedBoardsData = boardsData.filter(b => {
        console.log('Checking board:', b.board, 'against selected:', formData.board);
  ...
  useEffect(() => {
    if (formData.classes.length > 0) {
      ...
      const selectedClasses = availableClasses.filter(c => {
        console.log('Checking class:', c.name, 'against selected:', formData.classes);
        return formData.classes.includes(c.name);
      });
      
      console.log('Filtered Classes:', selectedClasses);
      
      const allSubjects = [...new Set(selectedClasses.flatMap(c => {
  ...
  const uploadFile = async (file: File): Promise<string> => {
    ...
    try {
      const urlRes = await fetch(`${baseUrl}/api/generateUrl?fileName=${file.name}&fileType=${file.type}`);
  ...
  const validate = () => {
    ...
    if (!formData.classes.length) newErrors.classes = 'Select at least one class';
    if (!formData.subjects.length) newErrors.subjects = 'Select at least one subject';
    if (!formData.experience) newErrors.experience = 'Experience is required';
    if (!formData.qualification) newErrors.qualification = 'Qualification is required';
    if (!isEdit && !selectedFile) newErrors.resumeLink = 'Resume file is required';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = isEdit ? `${baseUrl}/api/teachers/${initialData?._id}` : `${baseUrl}/api/addteacher`;
      ...
      if (!res.ok) {
        throw new Error(isEdit ? 'Failed to update teacher' : 'Failed to add teacher');
      }

      setSnackbar({
        open: true,
        message: isEdit ? 'Teacher updated successfully!' : 'Teacher added successfully!',
        severity: 'success'
      });
      ...
      setSnackbar({
        open: true,
        message: isEdit ? 'Failed to update teacher. Please try again.' : 'Failed to add teacher. Please try again.',
        severity: 'error'
      });
      if (onError) {
        onError();
    ...
    <Box component="form" onSubmit={handleSubmit} sx={{ p: 3, maxWidth: '1200px', margin: '0 auto' }}>
      <Paper elevation={3} sx={{ p: 4, borderRadius: 2 }}>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
          ...
          <Box sx={{ flex: '1 1 45%', minWidth: '300px' }}>
            <FormControl fullWidth error={!!errors.board} sx={{ minHeight: '80px' }}>
              ...
              <Select
                ...
                renderValue={selected => (
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, minHeight: '40px', alignItems: 'center' }}>
                    {selected.map(v => <Chip key={v} label={v} color="primary" size="medium" />)}
                  </Box>
                )}
                sx={{ minHeight: '60px' }}
              >
                {boardsData.map(board => (
                  <MenuItem key={board._id.$oid} value={board.board}>{board.board}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </Box>
          <Box sx={{ flex: '1 1 45%', minWidth: '300px' }}>
            <FormControl fullWidth error={!!errors.classes} sx={{ minHeight: '80px' }}>
              ...
              >
                {availableClasses.map(cls => (
                  <MenuItem key={cls._id.$oid} value={cls.name}>{cls.name}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </Box>
          <Box sx={{ flex: '1 1 45%', minWidth: '300px' }}>
            <FormControl fullWidth error={!!errors.subjects} sx={{ minHeight: '80px' }}>
          ...
          <Box sx={{ flex: '1 1 100%', display: 'flex', justifyContent: 'center', mt: 2 }}>
            <Button
              type="submit"
              variant="contained"
